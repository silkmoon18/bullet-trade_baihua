...
L00先将helper精简为安装契约；L03增加StrategyLedger短连接RPC和真实组合视图；当前API v14提供`JoinQuantRuntime`统一门面、买卖侧可独立声明的不可变执行请求、多策略配置v3、JQ/QMT双账户开关、未知费用视图，以及过期远程目标的完整终态处理。策略不再包含连接、协议、账户初始化/对账分支、重启恢复或QMT回调代码。
...
直连xtquant适配器已具备QMT订单/成交回调、资金/持仓对账、组合目标规划、条件行情触发和卖后买恢复；BigQMT HTTP gateway尚未桥接原生回调。尚缺的是目标QMT环境对稳定ID、费用和跨日查询的人工能力证明，以及聚宽JQ、QMT模拟和小额实盘运行证据。
...
回测继续BACKTEST原生运行；模拟交易可启用JQ、QMT或两者。双账户共享策略决策但分别使用自己的资金、持仓和成本；QMT启用时只发送QMT通知，否则维持JQ目标买入卡片。

当前策略只保留一次`install_joinquant_runtime`调用；账户开关解析、账户初始化、对账触发、API版本校验、状态发布、远程预检、通知容错和聚宽/QMT订单门面均由helper v14负责。策略交易入口只提交账户无关的权重与风控阈值，`initialize`与`process_initialize`的首条可执行语句均为runtime安装。权威账本和账实对账算法仍位于服务器StrategyLedger，helper只是聚宽侧门面。
