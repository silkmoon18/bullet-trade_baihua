...
| D022 | QMT能力必须由显式合同和运行探针确认；合成成交号、缺失费用默认为0或猜测成交方向均不能作为实盘证据 | Accepted |
| D023 | S11至S20改为四个个人量化纵向分片；先裁剪过重门禁和通用化，再完成单账户实盘闭环 | Accepted |
...
MiniQMT和BigQMT版本、券商柜台及网关返回字段并不完全一致。S06用统一能力合同描述client tag回显、稳定订单号/成交号、订单成交关联、费用、订单状态、working order和跨日查询范围。代码中存在某个字段名不等于真实环境已满足能力；最终必须由QMT模拟或小额实盘探针确认。

合成成交号可以用于诊断展示，但不能证明券商成交唯一性；缺少或无法解析费用字段时也不能把`0`解释为真实零费用，负费用同样无效。成交方向优先读取成交事实，缺失时只允许通过同一broker order ID映射已验证含side的订单，无法映射就阻断该成交入账，不按证券、数量或价格猜测。

## D023：剩余工作改为个人量化精简计划

原S11至S20偏向多租户、机构级运维和对抗性安全设计，不再逐项机械实现。首版固定为一个专用QMT账户、一个`good_etf`策略、单进程SQLite、固定初始1万元、禁止人工交易和运行中增减资。
