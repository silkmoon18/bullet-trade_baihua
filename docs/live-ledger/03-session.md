...
- MiniQMT包装账户响应在进入对账前统一解包。
- `SUBMIT_UNKNOWN`可按券商回传的完整`client_tag`备注唯一认领；不按证券、数量或价格猜测。
- 目标权重按总资产换算，现金缓冲仅在买入可负担性检查执行，修复双重缓冲和风控误卖未触发持仓。
- LIVE买卖限价分别相对行情上浮/下浮0.2%；工作订单超过10分钟时由下一次intent推进请求撤单。
- StrategyLedger请求超过旧QMT自动拆单阈值时明确拒绝，避免只保存第一笔券商订单号。
- 聚宽profile删除六个未使用配置和未使用的`get_events` RPC；内部账本事件仍保留。
- `production_ready`只在真实账户对账、组合读取和intent恢复成功后置为true。
- 相关测试277项通过；导出验证、mypy、Python 3.8语法和有效flake8检查通过。
- 尚未完成的实盘门槛仍是用户实际QMT费用字段/稳定ID/跨日查询证明，以及SHADOW、模拟和小额实盘人工验收。
