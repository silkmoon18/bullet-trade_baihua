...
- 统一仓库S00基线：压平后以当前分支相对`v0.9.2`的单个提交为准
...
- S00独立审查推动修复了API差异、忽略规则、迁移manifest、slice依赖和测试证据。返工历史曾短暂包含旧敏感审查特征，因此在未推送前将整个S00压平成单个脱敏提交。

## 当前状态

- 原 `bt_quant` 工作树干净，旧仓库保留。
- `.idea/`、runtime、导出产物和本地profile已有明确忽略规则。
- S00尚未标记DONE；历史压平后运行校验脚本，并以压平提交作为最终候选复审。
