...
> **结论：当前分支尚不能用于真实资金。** S00至S08完成，S09正在实现；S10至S17尚未实现；S18至S20的聚宽、QMT模拟和小额实盘门禁尚未通过。`LIVE`会失败关闭，这是设计行为。
...
1. 真实成交驱动的持仓批次、交易日状态、费用与已实现盈亏入账。
2. 部分成交/撤单/拒单入账、未知提交恢复和卖后买状态机（委托持久幂等及响应未知隔离已完成）。
