...
> **结论：L01至L03的软件闭环已实现，但当前分支仍不能直接用于真实资金。** 真实QMT能力证明、部署备份和人工验收属于L04；服务端交易开关默认关闭，静态QMT能力profile仍会让对账保持BLOCKED。
...
2. Windows服务启动、日志轮转、SQLite备份恢复和飞书交易卡片通知（L04）。
3. 聚宽SHADOW、QMT模拟和小额实盘的人工运行证据；真实资金必须再次明确批准。
...
代码路径已存在，但在L04验收前保持`QMT_STRATEGY_TRADING_ENABLED=false`。必须上传v2 helper和私有profile；启动时真实资金不足、能力未证明、账实差异或对账不新鲜都会失败关闭。配置与流程见[策略API与聚宽真实组合视图](19-strategy-api-joinquant-view.md)。
