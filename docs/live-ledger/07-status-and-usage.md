...
> **结论：当前分支尚不能用于真实资金。** S00至S09完成，S10正在实现；S11至S17尚未实现；S18至S20的聚宽、QMT模拟和小额实盘门禁尚未通过。`LIVE`会失败关闭，这是设计行为。
...
1. 基于可靠行情的持仓市值、总资产、NAV、收益与原子快照。
2. QMT订单/成交持续摄取、未知提交恢复、自动对账和卖后买执行状态机。
