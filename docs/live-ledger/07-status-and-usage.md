...
> **结论：当前分支尚不能用于真实资金。** S00至S10完成，S11正在实现；S12至S17尚未实现；S18至S20的聚宽、QMT模拟和小额实盘门禁尚未通过。`LIVE`会失败关闭，这是设计行为。
...
1. QMT订单、成交、资金和持仓的持续摄取、归属映射、未知提交恢复与自动对账。
2. 基于原子快照的目标组合规划和卖后买执行状态机。
