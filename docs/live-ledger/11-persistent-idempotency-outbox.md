...
首版面向个人单策略/单服务进程：outbox由服务器进程内嵌的单个dispatcher认领，没有外部worker，也没有lease；认领只区分“尚未跨过外部调用边界，可以重新投递”和“已经准备调用券商，结果必须视为未知”。不建设分布式消息平台。
...
1. `claim_next()`无参认领最早可投递的`PENDING`任务；认领在`BEGIN IMMEDIATE`事务内原子完成select-then-update，进程内并发调用不会领到同一行。
2. 紧邻QMT调用前执行`begin_submission(outbox_id)`，operation进入`SUBMITTING`。
3. 调用QMT一次。
4. 得到确定响应后调用`finish_submission(..., unknown=False)`，持久化响应并进入`COMPLETED`。
5. 请求已发出但响应丢失/超时时调用`finish_submission(..., unknown=True)`，进入`SUBMIT_UNKNOWN`且outbox置为终态`FAILED`，不再投递；未知提交不能重发，这是底线。

进程启动时先调用`quarantine_inflight()`。任何遗留`SUBMITTING`都转为`SUBMIT_UNKNOWN`，因为系统无法证明崩溃发生在券商收到请求之前；后续只能使用S06已验证的client tag/order查询路径恢复，不能再次发送原请求。崩溃前仅被认领（`CLAIMED`）但尚未开始提交的outbox行会重置回`PENDING`，允许dispatcher重新认领。

## 尚未接入

- 现有通用`RemoteBrokerApplication`仍使用短期内存幂等缓存；正式StrategyLedger API在L03切换到本repository。
- intent、资金冻结和订单行将在L02的专用事务入口中与operation/outbox组合（按订单冻结/释放服务已在S08完成），不能先提交业务再单独补outbox。
- `SUBMIT_UNKNOWN`自动查询与人工处置流程在L01/L02实现。
