...
`adjust_capital()`只接受`ALLOCATE`和`WITHDRAW`，同时更新物理资金池、策略现金、账本事件和`capital_flows`。`external_ref`必须稳定：相同ref和相同参数只返回当前账户而不重复生效，不同参数复用同一ref会报错。收回金额不能占用策略已冻结现金。

## 当前边界

- S08尚未从QMT adapter自动拉取账户资金；S11/S14负责接入服务器和启动顺序。
- S08尚未按成交更新现金/持仓；部分成交、手续费、卖出回款和T+1 lot在S09实现。
- 聚宽展示的真实资金、持仓和指标视图在S15/S16接入。在此之前`LIVE`仍保持阻断。
