...
服务端支持参考 `bilibili_notifier` 的飞书交互卡片。橙色表示策略目标计划，蓝色表示委托已提交，青色表示部分成交，绿色表示全部成交，灰色表示撤单，红色表示拒单、错误或对账阻断。卡片标题和正文均展示策略ID；正文同时固定展示标的、方向、状态、金额、数量、单价和时间，有值时还会展示订单号、成交号和费用说明。
