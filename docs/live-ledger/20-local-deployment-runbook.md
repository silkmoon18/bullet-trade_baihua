...
代码已具备本机运行入口、滚动日志、每日SQLite在线备份、恢复工具、飞书交易卡片和启动对账。真实QMT能力验证及SIGNAL_ONLY/QMT模拟/小额实盘仍必须在用户机器人工执行；mock测试不能替代这些证据。
...
配置Webhook后自动发送：`SIGNAL_ONLY/QMT_REMOTE`目标买入计划、委托提交、部分成交、全部成交、撤单、拒单和对账阻断。计划卡片汇总标的、本轮新增买入数量和总金额；交易卡片包含标的、方向、状态、金额、数量、单价、时间，以及可用时的订单号/成交号。通知发送失败只记为通知失败，不回滚真实账本。

## 7. 人工验收顺序

1. 聚宽`SIGNAL_ONLY`至少观察若干完整交易日：选股、权重、时间和真实只读快照符合预期。
