...
代码已具备本机运行入口、滚动日志、每日SQLite在线备份、恢复工具、飞书交易卡片和启动对账。真实QMT能力验证及SHADOW/模拟/小额实盘仍必须在用户机器人工执行；mock测试不能替代这些证据。
...
首版要求QMT专用现金账户，不允许人工交易或其他策略共用。建议把运行数据放在仓库外：
...
服务器启动后会对已存在策略账户立即同步QMT；失败时`strategy_ledger_ready=false`并保持只读。第一次由聚宽调用`ensure_account`建立1万元策略账户，同样必须返回`READY`。
...
配置Webhook后自动发送：委托提交、部分成交、全部成交、撤单、拒单和对账阻断。卡片包含标的、方向、状态、金额、数量、单价、时间，以及可用时的订单号/成交号。通知发送失败只记为通知失败，不回滚真实账本。

## 7. 人工验收顺序

1. 聚宽SHADOW至少观察若干完整交易日：选股、权重、时间和真实只读快照符合预期。
