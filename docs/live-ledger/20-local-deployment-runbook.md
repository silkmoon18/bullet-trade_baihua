...
券商连成交价和成交金额都缺失时，可在服务器私有`.data/.env`启用`QMT_STRATEGY_UNPRICED_FILL_POLICY=ZERO_FALLBACK`；同一规则适用于模拟与真实账户，正常有价成交路径不变。已确认的成交股数照常入账，估算单价优先取提交目标时的参考价，其次取委托限价；买入还可按本单预留资金估价，卖出还可按已有持仓成本估价。估算单价直接用于现金、持仓成本、盈亏和后续调仓，但标记为估算；参考 NAV/收益明确写“非精确收益”，不伪装成券商真实成交价。同一券商成交ID日后补回真实价格时，可原子更正现金、批次成本及已实现盈亏。默认`STRICT`不变。启用前备份账本，并先在模拟账户核实成交回报及重启恢复；详见[当前缺价口径](45-fill-price-recovery.md)，历史口径见[零价兜底记录](26-zero-price-fallback.md)和[估算记录](44-unpriced-fill-estimates.md)。
